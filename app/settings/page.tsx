...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const stored = typeof window !== "undefined" ? localStorage.getItem("BmbMusic_dl_server") || "" : ""
    setDlServer(stored)
  }, [])
      ...
      <div className="container max-w-2xl mx-auto px-4 py-8 pb-36 space-y-8">
        ...
        <section>
          ...
          <div className="rounded-2xl bg-gradient-to-br from-primary/20 via-primary/8 to-accent/10 border border-primary/20 p-5 mb-4 text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              ...
              />
              <h3 className="text-xl font-bold tracking-tight">BmbMusic</h3>
            </div>
          ...
          <p className="text-xs text-center text-muted-foreground/50 mt-4 px-2">
            BmbMusic is not affiliated with YouTube or Google. All music data is sourced from YouTube Music via an unofficial API for personal use.
          </p>
        </section>
        ...
        <section>
          <SectionHeader
            ...
            desc="Downloads work automatically for most songs. If they fail, you can run the included BmbMusic-downloader.js on any device — even your phone via Termux — and paste the URL here."
          />
          ...
          <div className="rounded-2xl bg-card/40 border border-border/30 p-4 mb-3">
              ...
              <Button
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (d.ok && d.ytdlp) {
                      localStorage.setItem("BmbMusic_dl_server", url)
                      setDlServer(url)
            ...
            {dlServerStatus === "fail" && (
              <div className="flex items-center gap-2 text-xs text-red-400">
                ...
                Could not reach the server. Check the URL and make sure BmbMusic-downloader.js is running.
              </div>
            )}
            {dlServer && dlServerStatus === "idle" && (
              <button
                onClick={() => { localStorage.removeItem("BmbMusic_dl_server"); setDlServer(""); setDlServerStatus("idle") }}
          ...
          <div className="rounded-2xl bg-card/20 border border-dashed border-border/40 p-4 space-y-2">
            ...
            <ol className="text-xs text-muted-foreground/80 space-y-1 list-decimal list-inside">
              ...
              <li>Copy <code className="bg-muted/60 rounded px-1">BmbMusic-downloader.js</code> to that device</li>
              <li>Run: <code className="bg-muted/60 rounded px-1">node BmbMusic-downloader.js</code></li>
