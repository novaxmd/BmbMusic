...
function getHostSecretId(partyId: string): string | null {
  ...
  try { return localStorage.getItem(`BmbSong_party_host_${partyId}`) || null } catch { return null }
}
...
export default function PartyGuestPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSearching(true)
    try {
      const res  = await fetch(`/api/bmbsong/search?q=${encodeURIComponent(query)}&filter=songs&limit=20`)
      const data = await res.json()
