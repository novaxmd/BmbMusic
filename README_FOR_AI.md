# BmbSong — Technical Reference for AI Assistants
...
```typescript
{
  _sig: string,           // HMAC with APP_SIG = "BmbSong_2025"
  _version: 1,
  user_id: string,
  songs: Record<songId, {
    play_count: number,
...
- **Host** (`app/party/[id]/host/page.tsx`): Controls playback. `partyHostId` is stored in `localStorage` key `BmbSong_party_host_<partyId>` and **never** sent over the network or exposed in `AudioContextType`.
