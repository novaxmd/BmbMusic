#!/usr/bin/env python3
"""
BmbSong Downloader — Python
================================
Implements the BmbMusic v7 download API so BmbSong can download songs
...
4. In BmbSong → Settings → Download Server, paste the public URL
...
log = logging.getLogger("BmbSong-dl")
    ...
    if thumbnail_url and ext in ("mp3", "m4a", "mp4"):
        try:
            ...
            req = urllib.request.Request(
                thumbnail_url,
                headers={
                    "User-Agent": "Mozilla/5.0 (compatible; BmbSong/1.0)",
...
def _run_download(uid: str, video_id: str, title: str, artist: str, album: str = "", thumbnail_url: str = "") -> None:
    ...
    out_tpl = os.path.join(tmp_dir, f"BmbSong_{uid}.%(ext)s")
    ...
    try:
        ...
        for f in Path(tmp_dir).iterdir():
            if f.name.startswith(f"BmbSong_{uid}."):
                found = f
                break
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _handle_health(self):
        ...
        self._send_json(200, {
            ...
            "server":   "BmbSong Downloader (Python)",
            "sessions": active,
        })
...
def main():
    ...
    print(f"    Then go to BmbSong → Settings → Download Server")
