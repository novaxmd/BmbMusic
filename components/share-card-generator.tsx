  ...
  const renderCard = useCallback(async (cardStyle: CardStyle, selStart: number, source: LyricSource) => {
    ...
    try {
      ...
      if (cardStyle === "lyrics") {
        ...
        ctx.textBaseline = "middle"
        ctx.fillText("BmbMusic", 48 + 50, brandY + 14)

        ctx.font      = `400 13px -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif`
        ctx.fillStyle = "rgba(255,255,255,0.25)"
        ctx.textAlign = "right"
        ctx.fillText("BmbMusic.vercel.app", W - 48, brandY + 14)
      ...
      ctx.textBaseline = "middle"
      ctx.fillText("BmbMusic", infoX + 52, brandY + 16)
  ...
  const handleSave = async () => {
    ...
    try {
      canvas.toBlob(async (blob) => {
        if (!blob) return
        const file = new File([blob], "BmbMusic-share.png", { type: "image/png" })
        if (navigator.canShare?.({ files: [file] })) {
          await navigator.share({ files: [file], title, text: `${title} — ${artist}` })
        } else {
          const url = URL.createObjectURL(blob)
          const a = document.createElement("a")
          a.href = url; a.download = "BmbMusic-share.png"; a.click()
