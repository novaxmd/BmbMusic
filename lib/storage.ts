/**
 * BmbMusic storage.ts  v2 — drop-in replacement
...
export function exportAllData(): void {
  ...
  try {
    ...
    const backup: BmbMusicBackup = { version: 1, exportedAt: Date.now(), data }
    const blob = new Blob([JSON.stringify(backup, null, 2)], { type: "application/json" })
    const url  = URL.createObjectURL(blob)
    const a    = document.createElement("a")
    a.href = url; a.download = `BmbMusic-backup-${new Date().toISOString().slice(0, 10)}.json`; a.click()
    URL.revokeObjectURL(url)
...
export function importAllData(
  ...
  try {
    const backup: BmbMusicBackup = JSON.parse(json)
