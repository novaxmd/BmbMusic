...
function startDownload(id, videoId, title, artist, album, thumbnailUrl) {
  ...
  const outBase = path.join(tmpDir, `BmbSong_${id}`)
...
function handleHealth(req, res) {
  ...
  res.end(JSON.stringify({
    ...
    server:   "BmbSong Downloader (Node.js)",
...
checkYtdlp().then(ok => {
  ...
  server.listen(PORT, "0.0.0.0", () => {
    ...
    console.log(`\n    Then go to BmbSong → Settings → Download Server`)
